/*
 * ~Deprecated~
 * Tried to make it scalable for as many elevators, but it became a little too complex
*/
...
export default class BuildingSystem {
	...
	call(floor: number, direction: 1 | -1) {
		...
		this.elevators.forEach((elevator, index) => {
			console.log(`Elevator current dir=${elevator.currentDirection}`)
		...
		// if it was idle, then immediately send it
		if (eleScores[elePicked] == 100) this.elevators[elePicked].goToNextFloor()
	}
